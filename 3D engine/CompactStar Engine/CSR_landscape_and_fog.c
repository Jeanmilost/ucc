/****************************************************************************
 * ==> Landscape and ground collision with a fog demo ----------------------*
 ****************************************************************************
 * Description : A landscape generator based on a grayscale image, with     *
 *               ground collision to walk on his surface, and a fog effect. *
 *               Swipe up or down to walk, and left or right to rotate      *
...
const char g_VSTextured[] =
    ...
    "varying   vec4 csr_vVertice;"
    "varying   vec4 csr_vColor;"
    "varying   vec2 csr_vTexCoord;"
    "varying   vec4 csr_vCamera;"
    "void main(void)"
    "{"
    "    csr_vVertice  = csr_aVertices * csr_uModel;"
    "    csr_vColor    = csr_aColor;"
    "    csr_vTexCoord = csr_aTexCoord;"
    "    csr_vCamera   = csr_uCamera;"
...
const char g_FSTextured[] =
    ...
    "void main(void)"
    "{"
    "    vec4  fogColor = vec4(0.48, 0.52, 0.5, 1.0);"
    "    float d        = distance(csr_vCamera, csr_vVertice);"
    "    float alpha    = getFogFactor(d);"
    "    gl_FragColor   = mix(csr_vColor * texture2D(csr_sColorMap, csr_vTexCoord), fogColor, alpha);"
    "}";
//------------------------------------------------------------------------------
...
void on_GLES2_Init(int view_w, int view_h)
{
    ...
    g_CamPosSlot              = glGetUniformLocation(g_pShader->m_ProgramID, "csr_uCamera");
...
void on_GLES2_Render()
{
    ...
    // calculate and apply the ground collision
    ApplyGroundCollision(&g_BoundingSphere, g_pTree, &g_ViewMatrix);

    GLfloat buffer[4] = {g_BoundingSphere.m_Center.m_X,
                         g_BoundingSphere.m_Center.m_Y,
                         g_BoundingSphere.m_Center.m_Z,
                         0.0f};

    // set the camera position in order to calculate the fog in shader
    glUniform4fv(g_CamPosSlot, 1, &buffer[0]);
